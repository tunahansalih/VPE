...
import torchvision
# Setup
from augmentations import Compose, Scale, CenterPadding, RandomHorizontallyFlip, RandomRotate
...
parser.add_argument('--dataset', type=str, default='belga2flickr',
                    help='dataset to use [gtsrb, belga2flickr, belga2toplogo]')  # for gtsrb2TT100K scenario, use main_train_test.py
...
def test(e, best_acc, val_trigger):
    ...
    f_iou.write(
        'epoch(%d), acc_cls: %04f  acc_trcls: %04f  acc_tecls: %04f  acc_all: %04f  top3: %04f  top5: %04f\n' % (
    ...
    if val_trigger:  # when validation performance higher than prev val_best performance.
        ...
        f_iou_class.write(
            'acc cls: %.4f  acc all: %.4f  rank mean: %.4f \n' % (acc_cls.mean(), acc_all, rank_all.mean()))
    ...
    if best_acc < acc_tecls.mean():  # update best score
        ...
        torch.save(net.state_dict(),
                   os.path.join('%s_testBest_net.pth' % args.dataset))  # if best_acc == 0, then valBest model is saved
